...
import { Door2UniversityCard, type Door2QueueRow } from "./Door2UniversityCard";
...
interface Door2CountrySectionProps {
...
export function Door2CountrySection({
  country,
  filters,
  selected,
  onToggleSelect,
  onOpenDrawer,
  onDataChange,
}: Door2CountrySectionProps) {
  const { t } = useTranslation("common");
  const { toast } = useToast();
  const { language } = useLanguage();
  const [isOpen, setIsOpen] = useState(false);
  const [universities, setUniversities] = useState<Door2QueueRow[]>([]);
  ...
  const fetchUniversities = useCallback(async (page: number, append = false) => {
    ...
    try {
      ...
      const { data, error } = await supabase.rpc("rpc_admin_door2_review_queue" as any, {
        p_filters: f,
        p_page: page,
