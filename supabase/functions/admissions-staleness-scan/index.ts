...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`[admissions-staleness-scan] Marked ${markedCount} consensus records as stale`);

    // Log telemetry
    await srv.from('events').insert({
      name: 'admissions_staleness_scan',
      visitor_id: 'system',
      properties: {
    ...
    console.error('[admissions-staleness-scan] Error:', error);
    return new Response(
