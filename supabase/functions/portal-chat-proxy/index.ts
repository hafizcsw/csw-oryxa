...
serve(async (req) => {
  ...
  if (req.method !== 'POST') {
    return errorResponse(405, 'portal_proxy_method_not_allowed');
  }
  ...
  try {
    ...
    if (!CRM_FUNCTIONS_URL || !ingressApiKey) {
      console.warn('[portal-chat-proxy] config_missing', { trace_id: traceId });
      return errorResponse(500, 'portal_proxy_not_configured');
    }
    ...
    try {
      ...
      return errorResponse(400, 'portal_proxy_invalid_json_body');
    }

    if (!body || Object.keys(body).length === 0) {
      return errorResponse(400, 'portal_proxy_empty_body');
    }
    ...
    const upstreamTarget = `${CRM_FUNCTIONS_URL}/assistant-process`;
    console.log('[portal-chat-proxy] upstream_request', {
      trace_id: traceId,
    ...
    const upstreamUrl = `${CRM_FUNCTIONS_URL}/assistant-process`;
    
    if (!upstream.ok) {
      const errBody = await upstream.text();
      console.error('[portal-chat-proxy] upstream_rejected', {
        trace_id: traceId,
        upstream_status: upstream.status,
        upstream_url: upstreamUrl,
        upstream_body: errBody?.slice(0, 500),
        sent_headers: Object.keys(buildIngressHeaders(req, ingressApiKey, traceId)),
      });
      return new Response(
        JSON.stringify({ ok: false, error_key: 'portal_proxy_upstream_error', upstream_status: upstream.status, upstream_detail: errBody?.slice(0, 200) }),
        { status: upstream.status, headers: jsonHeaders },
      );
    }

    console.log('[portal-chat-proxy] upstream_response', {
      trace_id: traceId,
      upstream_status: upstream.status,
    });
    ...
    console.error('[portal-chat-proxy] internal_error', { trace_id: traceId });
    return errorResponse(500, 'portal_proxy_internal_error');
  }
});
