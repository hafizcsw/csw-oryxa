...
async function sendEmailViaResend(payload: {
  ...
  if (!apiKey) {
    throw Object.assign(new Error('RESEND_API_KEY not configured'), { status: 500 })
  }
  ...
  if (payload.message_id) {
    body.headers = { 'X-Message-Id': payload.message_id }
  }
  ...
  if (!res.ok) {
    ...
    const err = Object.assign(new Error(`Resend error: ${res.status} ${responseBody}`), {
      status: res.status,
      retryAfterSeconds: res.status === 429
