...
const ROOT_DOMAIN = "cswworld.com"
const FROM_DOMAIN = "cswworld.com"

const SAMPLE_PROJECT_URL = "https://lavista-launchpad.lovable.app"
...
async function verifyHmacSignature(
  ...
  // Stale check: reject if older than 5 minutes
...
// --- Inline payload parser ---

function parseEmailWebhookPayload(bodyStr: string): any {
  const parsed = JSON.parse(bodyStr)
  if (!parsed.data) throw new Error('Missing data field in webhook payload')
  return parsed
}
...
async function handlePreview(req: Request): Promise<Response> {
  ...
  // Auth: accept either WEBHOOK_HMAC_SECRET or SUPABASE_SERVICE_ROLE_KEY
  const serviceKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')
  const hmacSecret = Deno.env.get('WEBHOOK_HMAC_SECRET')
  const authHeader = req.headers.get('Authorization')
  const token = authHeader?.replace(/^Bearer\s+/i, '')

  const authorized = token && (token === serviceKey || token === hmacSecret)
  if (!authorized) {
...
// Webhook handler - verifies HMAC signature and enqueues email
async function handleWebhook(req: Request): Promise<Response> {
  ...
  try {
    ...
    payload = parseEmailWebhookPayload(body)
  } catch (error) {
