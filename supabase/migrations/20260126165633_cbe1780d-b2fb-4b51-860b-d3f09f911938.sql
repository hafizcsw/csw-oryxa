-- Fix RPC to match actual view column names
-- View has: tuition_usd_year_min, tuition_usd_year_max, etc.
-- RPC was looking for: tuition_year_usd_min, tuition_year_usd_max, etc.
...
BEGIN
  ...
  -- ============================================================
  -- STEP 2: REJECT UNKNOWN KEYS (CORRECT SYNTAX)
  -- ============================================================
  ...
  -- Check tuition columns (CORRECT column names from view)
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'vw_program_search_api_v3_final' AND column_name = 'tuition_usd_year_min') THEN
    RETURN jsonb_build_object('ok', false, 'request_id', v_request_id, 'error', 'MISSING_DATA_FIELDS', 'missing_data_fields', '["column.tuition_usd_year_min"]'::jsonb, 'ignored_filters', '[]'::jsonb);
  END IF;
  ...
  -- ============================================================
  -- STEP 7: I18N FALLBACK
  -- ============================================================
  v_lang_primary := split_part(v_display_lang, '-', 1);

  -- ============================================================
  -- STEP 8: COUNT TOTAL (USING CORRECT COLUMN NAMES)
  -- ============================================================
  EXECUTE format($q$
    SELECT COUNT(*)
    ...
    WHERE v.is_active = true
      ...
      -- Partner filter (only = partner_tier IS NOT NULL)
      AND (
        CASE 
      ...
      AND (
        CASE %L
          WHEN 'year' THEN 
            ...
            OR (COALESCE(v.tuition_usd_year_min, 0) >= COALESCE(%L::numeric, 0) AND COALESCE(v.tuition_usd_year_max, v.tuition_usd_year_min, 0) <= COALESCE(%L::numeric, 999999999))
          WHEN 'semester' THEN
            (v.tuition_is_free = true AND 0 >= COALESCE(%L::numeric, 0) AND 0 <= COALESCE(%L::numeric, 999999999))
            OR (COALESCE(v.tuition_usd_semester_min, 0) >= COALESCE(%L::numeric, 0) AND COALESCE(v.tuition_usd_semester_max, v.tuition_usd_semester_min, 0) <= COALESCE(%L::numeric, 999999999))
          WHEN 'program_total' THEN
            (v.tuition_is_free = true AND 0 >= COALESCE(%L::numeric, 0) AND 0 <= COALESCE(%L::numeric, 999999999))
            OR (COALESCE(v.tuition_usd_program_total_min, 0) >= COALESCE(%L::numeric, 0) AND COALESCE(v.tuition_usd_program_total_max, v.tuition_usd_program_total_min, 0) <= COALESCE(%L::numeric, 999999999))
          ELSE true
        END
      )
  ...
  -- ============================================================
  -- STEP 9: FETCH ITEMS (FLAT RESPONSE, FREE=0)
  -- ============================================================
  EXECUTE format($q$
