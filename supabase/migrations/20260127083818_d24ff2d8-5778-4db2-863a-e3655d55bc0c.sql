-- ============================================
-- FIX: intake_months type mismatch (text[] vs integer[])
-- ============================================
...
DECLARE
  ...
  v_intake_months text[];  -- FIXED: text[] to match view
...
BEGIN
  ...
  -- FIXED: Intake months array - convert to text[] (view has text[])
  ...
  WITH filtered AS (
    ...
    WHERE 
      ...
      -- Tuition filter using _min columns based on basis
      AND (
      ...
      -- Scholarship filter using correct column name
      ...
      -- FIXED: Intake months filter (text[] overlap)
