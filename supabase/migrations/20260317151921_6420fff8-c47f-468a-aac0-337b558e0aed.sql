...
DECLARE
  ...
  -- Per-metric source counts for blending
  entity_view_count bigint;
  heartbeat_count bigint;
  route_uni_count bigint;
  route_prog_count bigint;
BEGIN
  -- Per-row source detection (NOT global switches)
  SELECT COUNT(*) INTO entity_view_count FROM events WHERE name='entity_view' AND created_at > c30d LIMIT 100;
  ...
  -- === FUNNEL with identity_domain labels ===
  ...
  FROM (
    ...
    FROM events WHERE name='page_view' AND created_at > c30d
    AND COALESCE(route,'') LIKE '%search%'
    UNION ALL
    ...
    UNION ALL
    SELECT 'university_view', 4, (
      -- Blend: entity_view events + route fallback for events without entity_view
      ...
      + COALESCE((
        SELECT COUNT(DISTINCT e.visitor_id)::int 
        FROM events e 
        WHERE e.name='page_view' AND e.created_at > c30d AND COALESCE(e.route,'') ~ '^/universities/[^/]+'
        AND NOT EXISTS (
          SELECT 1 FROM events e2 WHERE e2.name='entity_view' AND e2.visitor_id = e.visitor_id AND e2.properties->>'entity_type'='university' AND e2.created_at > c30d
        )
      ), 0)
    )::int, 'visitor_id'
    UNION ALL
    SELECT 'program_view', 5, (
      ...
      + COALESCE((
        ...
        AND NOT EXISTS (
          SELECT 1 FROM events e2 WHERE e2.name='entity_view' AND e2.visitor_id = e.visitor_id AND e2.properties->>'entity_type'='program' AND e2.created_at > c30d
        )
      ), 0)
    )::int, 'visitor_id'
    UNION ALL
  ...
  -- === UNIVERSITY INTEL (per-row blending, NOT global switch) ===
  SELECT jsonb_build_object(
    'top_by_views', COALESCE((
      SELECT jsonb_agg(row_to_json(r)::jsonb)
      FROM (
        -- Blend entity_view + route fallback per-visitor
        SELECT u.name as name_ar, u.name_en, u.slug, SUM(cnt) as views, COUNT(DISTINCT vid) as unique_visitors
        FROM (
          -- Source 1: entity_view events
          ...
          GROUP BY e.properties->>'entity_slug', e.visitor_id
          UNION ALL
          -- Source 2: route fallback for visitors WITHOUT entity_view
          SELECT REGEXP_REPLACE(COALESCE(e.route,''), '^/universities/([^/]+).*$', '\1') as uni_slug, e.visitor_id as vid, COUNT(*) as cnt
          FROM events e 
          WHERE e.name='page_view' AND e.created_at > c30d AND COALESCE(e.route,'') ~ '^/universities/[^/]+'
          AND NOT EXISTS (
            SELECT 1 FROM events e2 WHERE e2.name='entity_view' AND e2.visitor_id = e.visitor_id AND e2.properties->>'entity_type'='university' AND e2.created_at > c30d
          )
          GROUP BY uni_slug, e.visitor_id
        ) blended
    ...
    'top_by_shortlist', COALESCE((
      SELECT jsonb_agg(row_to_json(r)::jsonb)
      FROM (
        SELECT COALESCE(u.name, sl.eid) as name_ar, u.name_en, u.slug, u.id::text as entity_id,
               sl.adds, sl.unique_users
        FROM (
          SELECT COALESCE(properties->>'university_id', properties->>'entity_id') as eid,
          ...
          GROUP BY eid ORDER BY adds DESC LIMIT 20
        ) sl
        LEFT JOIN universities u ON u.id::text = sl.eid OR u.slug = sl.eid
        ORDER BY sl.adds DESC
      ) r
    ), '[]'),
    'top_programs_by_views', COALESCE((
      SELECT jsonb_agg(row_to_json(r)::jsonb)
      FROM (
        SELECT COALESCE(p.title, blended.prog_slug) as program_title, u.name as university_name,
               COALESCE(p.program_slug, blended.prog_slug) as prog_slug,
               SUM(blended.cnt) as views, COUNT(DISTINCT blended.vid) as unique_visitors
        FROM (
          -- Source 1: entity_view events
          ...
          GROUP BY prog_slug, e.visitor_id
          UNION ALL
          -- Source 2: route fallback for visitors WITHOUT entity_view
          SELECT REGEXP_REPLACE(COALESCE(e.route,''), '^/programs/([^/]+).*$', '\1') as prog_slug, e.visitor_id as vid, COUNT(*) as cnt
          FROM events e 
          WHERE e.name='page_view' AND e.created_at > c30d AND COALESCE(e.route,'') ~ '^/programs/[^/]+'
          AND NOT EXISTS (
            SELECT 1 FROM events e2 WHERE e2.name='entity_view' AND e2.visitor_id = e.visitor_id AND e2.properties->>'entity_type'='program' AND e2.created_at > c30d
        ...
        GROUP BY program_title, university_name, prog_slug ORDER BY views DESC LIMIT 20
      ) r
    ), '[]'),
    'data_source', CASE 
      WHEN entity_view_count > 0 THEN 'blended_entity_and_route'
      ELSE 'route_parsing_only'
    END
  ) INTO uni_intel;

  -- === SEARCH INTEL (visitor_id only, unified) ===
  SELECT jsonb_build_object(
    ...
    'search_to_click_pct', COALESCE((
      SELECT ROUND(100.0 * 
        (SELECT COUNT(DISTINCT visitor_id) FROM events WHERE name='search_result_click' AND created_at > c30d)::numeric /
        NULLIF((SELECT COUNT(DISTINCT visitor_id) FROM events WHERE name='search_performed' AND created_at > c30d), 0)
      )
    ), 0),
    'search_to_shortlist_pct', COALESCE((
      SELECT ROUND(100.0 *
        (SELECT COUNT(DISTINCT visitor_id) FROM events WHERE name IN ('shortlist_add','shortlist_added') AND created_at > c30d)::numeric /
        NULLIF((SELECT COUNT(DISTINCT visitor_id) FROM events WHERE name='search_performed' AND created_at > c30d), 0)
      )
    ), 0),
  ...
  SELECT jsonb_build_object(
    'universities_missing_tuition', COALESCE((
      ...
      FROM (
        ...
        FROM (
          SELECT uni_slug, SUM(cnt) as views FROM (
            SELECT properties->>'entity_slug' as uni_slug, COUNT(*) as cnt
            FROM events WHERE name='entity_view' AND properties->>'entity_type'='university' AND created_at > c30d
            GROUP BY uni_slug
            UNION ALL
            SELECT REGEXP_REPLACE(COALESCE(route,''), '^/universities/([^/]+).*$', '\1'), COUNT(*)
            FROM events WHERE name='page_view' AND created_at > c30d AND COALESCE(route,'') ~ '^/universities/[^/]+/?$'
            AND NOT EXISTS (SELECT 1 FROM events e2 WHERE e2.name='entity_view' AND e2.created_at > c30d AND e2.properties->>'entity_type'='university' LIMIT 1)
            GROUP BY 1
    ...
    'programs_missing_deadlines', COALESCE((
      ...
      FROM (
        ...
        FROM (
          SELECT prog_slug, SUM(cnt) as views FROM (
            SELECT properties->>'entity_slug' as prog_slug, COUNT(*) as cnt
            FROM events WHERE name='entity_view' AND properties->>'entity_type'='program' AND created_at > c30d
            GROUP BY prog_slug
            UNION ALL
            SELECT REGEXP_REPLACE(COALESCE(route,''), '^/programs/([^/]+).*$', '\1'), COUNT(*)
            FROM events WHERE name='page_view' AND created_at > c30d AND COALESCE(route,'') ~ '^/programs/[^/]+/?$'
            AND NOT EXISTS (SELECT 1 FROM events e2 WHERE e2.name='entity_view' AND e2.created_at > c30d AND e2.properties->>'entity_type'='program' LIMIT 1)
            GROUP BY 1
