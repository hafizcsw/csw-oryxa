...
-- Fix B: Update RPCs to exclude "empty" universities (pending + 0 drafts) by default
-- Add p_filters->>'include_empty' = 'true' to override
...
DECLARE
  ...
  v_include_empty boolean := coalesce((p_filters->>'include_empty')::boolean, false);
  v_total int;
  v_rows jsonb;
BEGIN
  ...
  WHERE (v_status IS NULL OR u.crawl_status = v_status)
    ...
    -- Fix B: exclude empty pending universities unless include_empty=true
    AND (v_include_empty OR NOT (
      u.crawl_status = 'pending'
      AND NOT EXISTS (SELECT 1 FROM program_draft pd WHERE pd.university_id = u.id)
    ));
...
-- Same fix for country summary RPC
CREATE OR REPLACE FUNCTION public.rpc_get_review_countries(
  p_filters jsonb DEFAULT '{}'::jsonb
)
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER
