...
BEGIN
  -- Step 1: Aggregate best observation per (entity_id, field_name) using highest confidence
  FOR v_rec IN
    WITH ranked AS (
    ...
    SELECT * FROM ranked
  LOOP
    -- Check if this program was already touched in this run (aggregate fields into one draft)
    IF v_rec.entity_id = ANY(v_programs_touched) THEN
      -- Update existing draft row's extracted_json and field_evidence_map
      ...
      SET extracted_json = COALESCE(extracted_json, '{}'::jsonb) || jsonb_build_object(v_field_key, v_rec.value_raw),
          field_evidence_map = COALESCE(field_evidence_map, '{}'::jsonb) || jsonb_build_object(
            v_field_key, jsonb_build_object(
              'obs_id', v_rec.obs_id,
              'quote', left(v_rec.evidence_snippet, 200),
          ...
          extractor_version = v_rec.parser_version
      WHERE published_program_id = v_rec.entity_id
        AND schema_version = 'osc_language_v1'
        AND review_status = 'draft'
      RETURNING id INTO v_draft_id;

      IF NOT _dry_run AND v_draft_id IS NULL THEN
        -- Race condition: skip
      ...
      v_details := v_details || jsonb_build_object(
        ...
        'draft_id', v_draft_id, 'action', CASE WHEN _dry_run THEN 'would_merge' ELSE 'merged' END
      );
      CONTINUE;
    END IF;

    -- First field for this program: create a new draft row
    v_field_key := v_rec.field_name;
    ...
    -- Check for existing draft for this program with same schema
    ...
    IF v_existing_draft_id IS NOT NULL THEN
      ...
      IF NOT _dry_run THEN
        UPDATE program_draft
        SET extracted_json = v_extracted,
            field_evidence_map = v_evidence,
            source_url = v_rec.source_url,
            last_extracted_at = now(),
            extractor_version = v_rec.parser_version,
            confidence_score = v_rec.confidence
    ...
    ELSE
      ...
      IF NOT _dry_run THEN
        INSERT INTO program_draft (
          title, university_name, university_id, country_code,
          degree_level, source_url, schema_version, status, review_status,
          extracted_json, field_evidence_map, confidence_score,
          published_program_id, last_extracted_at, extractor_version
        ) VALUES (
          v_rec.program_title,
          v_rec.university_name,
          v_rec.university_id,
          ...
          v_rec.entity_id,
          now(),
          v_rec.parser_version
        ) RETURNING id INTO v_draft_id;
      ELSE
        v_draft_id := -1; -- placeholder for dry run
      END IF;
    END IF;
