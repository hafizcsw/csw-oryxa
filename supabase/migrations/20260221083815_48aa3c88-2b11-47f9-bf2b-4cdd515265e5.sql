-- Temporarily disable ALL publish gate triggers to fix existing data
ALTER TABLE programs DISABLE TRIGGER trg_enforce_program_publish_v3;
ALTER TABLE programs DISABLE TRIGGER trg_validate_program_publish;
ALTER TABLE programs DISABLE TRIGGER trg_validate_program_publish_insert;

-- Update existing programs to published status
UPDATE public.programs 
SET publish_status = 'published', published = true 
WHERE is_active = true AND publish_status = 'draft';

-- Re-enable triggers
ALTER TABLE programs ENABLE TRIGGER trg_enforce_program_publish_v3;
ALTER TABLE programs ENABLE TRIGGER trg_validate_program_publish;
ALTER TABLE programs ENABLE TRIGGER trg_validate_program_publish_insert;

-- Fix the RPC to use correct trigger names and set publish_status='published'
