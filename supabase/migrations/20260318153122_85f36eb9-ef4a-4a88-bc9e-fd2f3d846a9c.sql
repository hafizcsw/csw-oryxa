CREATE OR REPLACE FUNCTION public.rpc_we_pick_batch(p_job_id uuid, p_batch_size int DEFAULT 500)
RETURNS int
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Preflight guard: verify no rows were seeded with NULL university_name
  ...
  IF v_null_name_count > 0 THEN
    -- Delete invalid rows and reduce inserted count
