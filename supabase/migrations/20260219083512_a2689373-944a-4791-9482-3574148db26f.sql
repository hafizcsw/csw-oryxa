...
-- Fix type cast for program_urls join
...
BEGIN
  ...
  FROM uniranks_crawl_state WHERE stage = 'done' AND updated_at >= now() - interval '15 minutes';

  SELECT COUNT(*) INTO v_errors_15m
  FROM uniranks_crawl_state WHERE quarantine_reason IS NOT NULL AND updated_at >= now() - interval '15 minutes';

  SELECT COUNT(*) INTO v_pending_total
  FROM uniranks_crawl_state WHERE stage IN ('profile_pending','programs_pending','details_pending');
  ...
  FROM (
    SELECT jsonb_build_object(
      ...
      'program_links_count', (SELECT count(*) FROM program_urls pu WHERE pu.university_id = cs.university_id::uuid)
    ) AS row_data
    FROM uniranks_crawl_state cs
    JOIN universities u ON u.id::text = cs.university_id
    WHERE cs.updated_at >= v_since
