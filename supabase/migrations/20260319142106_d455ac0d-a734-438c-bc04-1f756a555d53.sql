DROP FUNCTION IF EXISTS public.rpc_publish_program_batch_search(UUID);
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- PATCHED: validate apply_url against official domain family
      v_apply_url := NULL;
      v_candidate_url := rec.extracted_json->>'apply_url';
      IF v_candidate_url IS NOT NULL AND public.is_official_domain_url(v_candidate_url, rec.university_id) THEN
        v_apply_url := v_candidate_url;
      END IF;
      IF v_apply_url IS NULL THEN
        v_candidate_url := rec.extracted_json->>'detail_url';
        IF v_candidate_url IS NOT NULL AND public.is_official_domain_url(v_candidate_url, rec.university_id) THEN
          v_apply_url := v_candidate_url;
        END IF;
      END IF;
      IF v_apply_url IS NULL THEN
        IF rec.source_program_url IS NOT NULL AND public.is_official_domain_url(rec.source_program_url, rec.university_id) THEN
          v_apply_url := rec.source_program_url;
        END IF;
      END IF;
      ...
      INSERT INTO programs (
        ...
        apply_url, ielts_min_overall, ielts_required, duolingo_min, pte_min, cefr_level,
        ...
        v_apply_url, v_ielts, v_ielts_flag, v_duolingo, v_pte, v_cefr,
        'draft', true, false
