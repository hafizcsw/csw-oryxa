-- ============================================
-- FIX: kb_search_v1_3_final - Contract Alignment
-- ============================================
-- 1. Language filter: instruction_languages (text[]) ONLY
-- 2. Response Shape: FLAT (no nested objects)
-- 3. enforce_eligibility: MANDATORY true
-- 4. Reject unknown: proper missing_data_fields[]
-- ============================================
...
BEGIN
  ...
  IF v_unknown_keys IS NOT NULL AND array_length(v_unknown_keys, 1) > 0 THEN
    FOREACH v_tuition_basis IN ARRAY v_unknown_keys LOOP
      v_missing_fields := array_append(v_missing_fields, 'unsupported_filter.' || v_tuition_basis);
    END LOOP;
  END IF;
  ...
  IF v_unknown_keys IS NOT NULL AND array_length(v_unknown_keys, 1) > 0 THEN
    FOREACH v_tuition_basis IN ARRAY v_unknown_keys LOOP
      v_missing_fields := array_append(v_missing_fields, 'unsupported_admission.' || v_tuition_basis);
    END LOOP;
  END IF;
  ...
  -- ============================================
  -- 6. APPLICANT PROFILE VALIDATION (for eligibility)
  -- ============================================
  IF v_enforce_eligibility = true THEN
    v_curriculum := v_applicant->>'curriculum';
    v_stream := v_applicant->>'stream';
    v_citizenship := v_applicant->>'citizenship_country_code';
  ...
  WITH filtered AS (
    SELECT 
      ...
      CASE v_tuition_basis
        WHEN 'year' THEN COALESCE(v.tuition_year_usd_min, v.tuition_usd_min)
        WHEN 'semester' THEN COALESCE(v.tuition_semester_usd_min, v.tuition_usd_min)
        WHEN 'program_total' THEN COALESCE(v.tuition_total_usd_min, v.tuition_usd_min)
        ELSE v.tuition_usd_min
      END AS tuition_basis_min,
      CASE v_tuition_basis
        WHEN 'year' THEN COALESCE(v.tuition_year_usd_max, v.tuition_usd_max)
        WHEN 'semester' THEN COALESCE(v.tuition_semester_usd_max, v.tuition_usd_max)
        WHEN 'program_total' THEN COALESCE(v.tuition_total_usd_max, v.tuition_usd_max)
        ELSE v.tuition_usd_max
      ...
      END AS display_name,
      CASE 
        WHEN v.university_name_i18n ? v_display_lang THEN v.university_name_i18n->>v_display_lang
        WHEN v.university_name_i18n ? v_lang_primary THEN v.university_name_i18n->>v_lang_primary
        WHEN v.university_name_i18n ? 'en' THEN v.university_name_i18n->>'en'
        ELSE v.university_name_i18n->>'ar'
    ...
    WHERE v.is_active = true
      ...
      AND (
        ...
        OR (v_partner_priority = 'prefer') -- handled in ORDER BY
      )
      -- Optional filters
