...
BEGIN
  ...
  -- FUNNEL with identity_domain labels and separate search steps
  ...
  FROM (
    ...
    SELECT 'university_view', 4, (
      COALESCE((SELECT COUNT(DISTINCT visitor_id) FROM events WHERE name='entity_view' AND properties->>'entity_type'='university' AND created_at > c30d), 0)
      + COALESCE((SELECT COUNT(DISTINCT e.visitor_id) FROM events e WHERE e.name='page_view' AND e.created_at > c30d AND COALESCE(e.route,'') ~ '^/universities/[^/]+'
        AND NOT EXISTS (SELECT 1 FROM events e2 WHERE e2.name='entity_view' AND e2.visitor_id = e.visitor_id AND e2.properties->>'entity_type'='university' AND e2.created_at > c30d)), 0)
    )::int, 'visitor_id'
    UNION ALL
    SELECT 'program_view', 5, (
      COALESCE((SELECT COUNT(DISTINCT visitor_id) FROM events WHERE name='entity_view' AND properties->>'entity_type'='program' AND created_at > c30d), 0)
      + COALESCE((SELECT COUNT(DISTINCT e.visitor_id) FROM events e WHERE e.name='page_view' AND e.created_at > c30d AND COALESCE(e.route,'') ~ '^/programs/[^/]+'
        AND NOT EXISTS (SELECT 1 FROM events e2 WHERE e2.name='entity_view' AND e2.visitor_id = e.visitor_id AND e2.properties->>'entity_type'='program' AND e2.created_at > c30d)), 0)
    )::int, 'visitor_id'
    UNION ALL
  ...
  -- UNIVERSITY INTEL (per-visitor blending)
  SELECT jsonb_build_object(
    'top_by_views', COALESCE((
      SELECT jsonb_agg(row_to_json(r)::jsonb) FROM (
        ...
        FROM (
          SELECT e.properties->>'entity_slug' as uni_slug, e.visitor_id as vid, COUNT(*) as cnt FROM events e WHERE e.name='entity_view' AND e.properties->>'entity_type'='university' AND e.created_at > c30d GROUP BY 1, 2
          UNION ALL
          SELECT REGEXP_REPLACE(COALESCE(e.route,''), '^/universities/([^/]+).*$', '\1'), e.visitor_id, COUNT(*) FROM events e WHERE e.name='page_view' AND e.created_at > c30d AND COALESCE(e.route,'') ~ '^/universities/[^/]+' AND NOT EXISTS (SELECT 1 FROM events e2 WHERE e2.name='entity_view' AND e2.visitor_id = e.visitor_id AND e2.properties->>'entity_type'='university' AND e2.created_at > c30d) GROUP BY 1, 2
        ) blended
    ...
    'top_programs_by_views', COALESCE((
      SELECT jsonb_agg(row_to_json(r)::jsonb) FROM (
        ...
        FROM (
          SELECT COALESCE(e.properties->>'entity_slug', e.properties->>'entity_id') as prog_slug, e.visitor_id as vid, COUNT(*) as cnt FROM events e WHERE e.name='entity_view' AND e.properties->>'entity_type'='program' AND e.created_at > c30d GROUP BY 1, 2
          UNION ALL
          SELECT REGEXP_REPLACE(COALESCE(e.route,''), '^/programs/([^/]+).*$', '\1'), e.visitor_id, COUNT(*) FROM events e WHERE e.name='page_view' AND e.created_at > c30d AND COALESCE(e.route,'') ~ '^/programs/[^/]+' AND NOT EXISTS (SELECT 1 FROM events e2 WHERE e2.name='entity_view' AND e2.visitor_id = e.visitor_id AND e2.properties->>'entity_type'='program' AND e2.created_at > c30d) GROUP BY 1, 2
        ) blended
    ...
    'data_source', CASE WHEN entity_view_count > 0 THEN 'blended_entity_and_route' ELSE 'route_parsing_only' END
  ) INTO uni_intel;

  -- SEARCH INTEL (visitor_id only)
  SELECT jsonb_build_object(
      ...
      SELECT jsonb_agg(row_to_json(r)::jsonb) FROM (
        SELECT u.name as name_ar, u.slug, uv.views FROM (
          SELECT uni_slug, SUM(cnt)::bigint as views FROM (
            SELECT properties->>'entity_slug' as uni_slug, COUNT(*) as cnt FROM events WHERE name='entity_view' AND properties->>'entity_type'='university' AND created_at > c30d GROUP BY 1
            UNION ALL
            SELECT REGEXP_REPLACE(COALESCE(route,''), '^/universities/([^/]+).*$', '\1'), COUNT(*) FROM events WHERE name='page_view' AND created_at > c30d AND COALESCE(route,'') ~ '^/universities/[^/]+/?$' AND entity_view_count = 0 GROUP BY 1
      ...
      SELECT jsonb_agg(row_to_json(r)::jsonb) FROM (
        SELECT p.title, u.name as uni_name, u.slug as uni_slug, pv.views FROM (
          SELECT prog_slug, SUM(cnt)::bigint as views FROM (
            SELECT properties->>'entity_slug' as prog_slug, COUNT(*) as cnt FROM events WHERE name='entity_view' AND properties->>'entity_type'='program' AND created_at > c30d GROUP BY 1
            UNION ALL
            SELECT REGEXP_REPLACE(COALESCE(route,''), '^/programs/([^/]+).*$', '\1'), COUNT(*) FROM events WHERE name='page_view' AND created_at > c30d AND COALESCE(route,'') ~ '^/programs/[^/]+/?$' AND entity_view_count = 0 GROUP BY 1
