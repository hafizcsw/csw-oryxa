...
BEGIN
  ...
  IF v_draft.schema_version != 'osc_language_v1' THEN
    RETURN jsonb_build_object('ok', false, 'error', 'wrong schema_version');
  END IF;

  IF v_draft.published_at IS NOT NULL THEN
    RETURN jsonb_build_object('ok', false, 'error', 'already published');
  END IF;
  ...
  IF v_new_ielts IS NOT NULL THEN
    v_changes := v_changes || jsonb_build_object('ielts_min_overall',
      jsonb_build_object('old', to_jsonb(v_old.ielts_min_overall), 'new', to_jsonb(v_new_ielts)));
    v_fem := jsonb_set(v_fem, '{min_ielts,old_value}', to_jsonb(v_old.ielts_min_overall));
  END IF;

  IF v_new_toefl IS NOT NULL THEN
    v_changes := v_changes || jsonb_build_object('toefl_min',
      jsonb_build_object('old', to_jsonb(v_old.toefl_min), 'new', to_jsonb(v_new_toefl)));
    v_fem := jsonb_set(v_fem, '{min_toefl,old_value}', to_jsonb(v_old.toefl_min));
  END IF;

  IF v_new_duolingo IS NOT NULL THEN
    v_changes := v_changes || jsonb_build_object('duolingo_min',
      jsonb_build_object('old', to_jsonb(v_old.duolingo_min), 'new', to_jsonb(v_new_duolingo)));
    v_fem := jsonb_set(v_fem, '{duolingo_min,old_value}', to_jsonb(v_old.duolingo_min));
  END IF;

  IF v_new_pte IS NOT NULL THEN
    v_changes := v_changes || jsonb_build_object('pte_min',
      jsonb_build_object('old', to_jsonb(v_old.pte_min), 'new', to_jsonb(v_new_pte)));
    v_fem := jsonb_set(v_fem, '{pte_min,old_value}', to_jsonb(v_old.pte_min));
  END IF;

  IF v_new_cefr IS NOT NULL THEN
    v_changes := v_changes || jsonb_build_object('cefr_level',
      jsonb_build_object('old', to_jsonb(v_old.cefr_level), 'new', to_jsonb(v_new_cefr)));
  ...
  UPDATE programs SET
    ...
    ielts_required    = CASE WHEN v_new_ielts IS NOT NULL THEN 1 ELSE ielts_required END,
    toefl_min         = COALESCE(v_new_toefl, toefl_min),
    toefl_required    = CASE WHEN v_new_toefl IS NOT NULL THEN 1 ELSE toefl_required END,
