...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- Compute intake_months and next_intake_date
      v_intake_months := COALESCE(rec.intake_months, ARRAY[8]);
      
      -- Compute next intake date from intake_months
      v_next_intake := NULL;
      IF array_length(v_intake_months, 1) > 0 THEN
        -- Find the next intake month from now
        DECLARE m INT; today DATE := CURRENT_DATE;
        BEGIN
          FOREACH m IN ARRAY v_intake_months LOOP
          ...
          IF v_next_intake IS NULL THEN
            v_next_intake := make_date(EXTRACT(YEAR FROM today)::INT + 1, v_intake_months[1], 1);
          END IF;
        END;
      END IF;
