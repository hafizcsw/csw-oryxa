-- Fix column name mismatch in 3 RPCs: entity_type/entity_id/row_id → target_university_id/target_row_id

CREATE OR REPLACE FUNCTION public.rpc_publish_verified_university_geo(
  p_row_id uuid, p_actor_id uuid, p_trace_id text, p_reason text DEFAULT NULL
)
...
BEGIN
  ...
  SELECT to_jsonb(u.*) INTO v_before FROM universities u WHERE u.id = v_row.university_id;
  ...
  INSERT INTO geo_verification_decisions (
    decision_type, actor_id, target_university_id, target_row_id,
    trace_id, reason, before_state, after_state
  ) VALUES (
    'approve_geo', p_actor_id, v_row.university_id, p_row_id,
...
BEGIN
  ...
  INSERT INTO geo_verification_decisions (
    ...
    'reject_geo', p_actor_id, v_row.university_id, p_row_id,
    p_trace_id, p_reason,
...
CREATE OR REPLACE FUNCTION public.rpc_force_publish_geo_after_manual_review(
  p_row_id uuid, p_actor_id uuid, p_trace_id text, p_reason text,
  p_override_lat double precision DEFAULT NULL, p_override_lon double precision DEFAULT NULL
)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $function$
DECLARE
  v_row geo_verification_rows%ROWTYPE;
...
BEGIN
  ...
  INSERT INTO geo_verification_decisions (
    ...
    'force_publish_geo', p_actor_id, v_row.university_id, p_row_id,
    p_trace_id, p_reason, v_before,
