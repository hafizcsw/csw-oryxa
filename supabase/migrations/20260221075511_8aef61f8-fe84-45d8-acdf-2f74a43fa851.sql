...
-- Fix rpc_admin_door2_review_countries to only show universities with program drafts
...
BEGIN
  ...
  -- Counters (only for universities with drafts)
  SELECT jsonb_build_object(
    'processed_15m', (SELECT count(*) FROM public.uniranks_crawl_state cs WHERE cs.updated_at >= now() - interval '15 minutes' AND (v_run_id IS NULL OR cs.door2_run_id = v_run_id) AND EXISTS (SELECT 1 FROM public.program_draft pd WHERE pd.university_id::text = cs.university_id AND pd.schema_version = 'door2-detail-v1')),
    ...
    'pending_total', (SELECT count(*) FROM public.uniranks_crawl_state cs WHERE cs.stage IN ('profile_pending','programs_pending') AND (v_run_id IS NULL OR cs.door2_run_id = v_run_id) AND EXISTS (SELECT 1 FROM public.program_draft pd WHERE pd.university_id::text = cs.university_id AND pd.schema_version = 'door2-detail-v1'))
  ) INTO v_counters;

  -- Countries — only universities that have Door 2 program drafts
  WITH filtered AS (
...
-- Fix rpc_admin_door2_review_queue to only show universities with program drafts
...
BEGIN
  ...
  WITH filtered AS (
    SELECT
      ...
      COALESCE((SELECT count(*) FROM public.program_draft pd WHERE pd.university_id = u.id AND pd.schema_version = 'door2-detail-v1'), 0)::int AS program_draft_count,
    ...
    LEFT JOIN public.countries c ON c.id = u.country_id
    LEFT JOIN LATERAL (
      SELECT sr.status FROM public.uniranks_step_runs sr
