...
begin
  v := jsonb_build_object(
    ...
    -- Bot response latency metrics (from properties JSON)
    ...
    -- Active users (visited in last 5 minutes)
    ...
    -- Top routes (from properties->route)
    'top_routes_24h', coalesce((
      select jsonb_agg(row_to_json(t))
      from (
        select properties->>'route' as route, count(*) as pageviews
        from events
        where name = 'page_view' 
        and created_at >= now() - interval '24 hours'
    ...
    -- Recent events (using properties for route/tab)
    'events_recent', (
      ...
      from (select id, name, created_at, properties from events order by created_at desc limit 15) sub
    ),
