CREATE OR REPLACE FUNCTION public.rpc_publish_program_batch_search(p_batch_id text)
RETURNS TABLE(published int, skipped int, errors int)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- Extract program-level fields (no university fallback)
      ...
      -- FIX: ielts_required is a 0/1 flag, not a score
