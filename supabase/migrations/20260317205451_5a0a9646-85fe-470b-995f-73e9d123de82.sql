...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'returning_visitors_pct', COALESCE((
      SELECT round(count(*)::numeric / NULLIF((SELECT count(distinct visitor_id) FROM events WHERE created_at >= _cutover AND COALESCE(traffic_class,'real') IN ('real','known_real') AND COALESCE(is_admin,false)=false AND COALESCE(is_test,false)=false AND COALESCE(is_staff,false)=false AND created_at >= now()-interval '30 days'), 0) * 100)
  ...
  -- ============ ENGAGEMENT ============
  -- Use COALESCE(route, properties->>'route') to handle both old and new page_view events
  SELECT jsonb_build_object(
  ...
  SELECT jsonb_build_object(
    ...
    'top_by_views', COALESCE((SELECT jsonb_agg(row_to_json(u) ORDER BY u.views DESC) FROM (
      ...
      LEFT JOIN universities uni ON uni.id::text = sub.entity_id OR uni.slug = sub.entity_slug
    ...
    'top_programs_by_views', COALESCE((SELECT jsonb_agg(row_to_json(p2) ORDER BY p2.views DESC) FROM (
      ...
      LEFT JOIN programs prog ON prog.id::text = sub.entity_id OR prog.slug = sub.entity_slug
  ...
  -- ============ SEARCH INTELLIGENCE ============
  -- Use country_slug key since that's what the UI sends
  ...
  SELECT jsonb_build_object(
    ...
    'programs_missing_deadlines', COALESCE((SELECT jsonb_agg(row_to_json(p2)) FROM (SELECT COALESCE(prog.title_ar, prog.title, prog.slug, prog.id::text) as title, COALESCE(uni.name_ar, uni.name_en, uni.slug, uni.id::text) as uni_name, uni.slug as uni_slug, 0 as views FROM programs prog JOIN universities uni ON uni.id = prog.university_id WHERE prog.deadlines IS NULL ORDER BY uni.name_ar NULLS LAST LIMIT 10) p2), '[]'::jsonb),
