...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'programs_missing_deadlines', COALESCE((SELECT jsonb_agg(row_to_json(p2)) FROM (SELECT COALESCE(prog.title_ar, prog.title, prog.id::text) as title, uni.name_ar as uni_name, uni.id::text as uni_slug, 0 as views FROM programs prog JOIN universities uni ON uni.id = prog.university_id WHERE prog.deadlines IS NULL ORDER BY uni.name_ar LIMIT 10) p2), '[]'::jsonb),
