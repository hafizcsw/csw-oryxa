...
-- Fix programs table reference: program_slug not slug
...
BEGIN
  ...
  FROM (
    ...
    UNION ALL
    SELECT 'search', 2, count(distinct ev.visitor_id), 'visitor_id'
    FROM events ev WHERE (ev.name = 'search_performed' OR (ev.name = 'page_view' AND ev.route = '/search'))
      AND ev.created_at >= _cutover AND ev.traffic_class = 'real' AND ev.is_admin = false AND ev.is_test = false
    UNION ALL
    SELECT 'university_view', 3, count(distinct ev.visitor_id), 'visitor_id'
    FROM events ev WHERE ((ev.name = 'entity_view' AND ev.properties->>'entity_type' = 'university')
      ...
      AND ev.created_at >= _cutover AND ev.traffic_class = 'real' AND ev.is_admin = false AND ev.is_test = false
    UNION ALL
    SELECT 'program_view', 4, count(distinct ev.visitor_id), 'visitor_id'
    FROM events ev WHERE ((ev.name = 'entity_view' AND ev.properties->>'entity_type' = 'program')
      ...
      AND ev.created_at >= _cutover AND ev.traffic_class = 'real' AND ev.is_admin = false AND ev.is_test = false
    UNION ALL
    SELECT 'shortlist', 5, count(distinct ev.visitor_id), 'visitor_id'
    FROM events ev WHERE ev.name in ('shortlist_add','shortlist_added')
      AND ev.created_at >= _cutover AND ev.traffic_class = 'real' AND ev.is_admin = false AND ev.is_test = false
    UNION ALL
    SELECT 'register_start', 6, count(distinct ev.visitor_id), 'visitor_id'
    FROM events ev WHERE ev.name = 'register_start'
      AND ev.created_at >= _cutover AND ev.traffic_class = 'real' AND ev.is_admin = false AND ev.is_test = false
    UNION ALL
    SELECT 'register_complete', 7, (select count(*) from profiles where created_at >= _cutover), 'user_id'
    UNION ALL
    SELECT 'apply_click', 8, (select count(distinct user_id) from applications where created_at >= _cutover), 'user_id'
    UNION ALL
    SELECT 'doc_upload', 9,
  ...
  SELECT jsonb_build_object(
    'top_by_views', coalesce((
      ...
      from (
        ...
        group by c.slug, u.name_ar, u.name_en order by views desc limit 10
      ) t
    ), '[]'::jsonb),
    ...
    'top_programs_by_views', coalesce((
      ...
      from (
        ...
        where c.slug is not null and c.slug != ''
        group by c.slug, p.title order by views desc limit 10
      ) t
    ), '[]'::jsonb),
    ...
    'universities_missing_tuition', coalesce((
      ...
      from (
        ...
        where ev.name = 'page_view' and ev.route like '/universities/%'
          and ev.created_at >= _cutover and ev.traffic_class = 'real' and ev.is_admin = false and ev.is_test = false
          and not exists (select 1 from programs p where p.university_id = u.id and p.tuition_fees_usd is not null)
        group by u.name_ar, u.slug having count(*) >= 2 order by 3 desc limit 10
      ) t
    ), '[]'::jsonb),
