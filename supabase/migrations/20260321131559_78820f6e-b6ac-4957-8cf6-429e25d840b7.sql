...
-- One-time bulk publish of remaining unpublished drafts (qs_bridge_v1 + door5-programs-v1)
-- This does exactly what rpc_publish_programs does but without auth.uid() check
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      IF v_ielts IS NOT NULL THEN v_ielts_flag := 1; ELSE v_ielts_flag := NULL; END IF;
      ...
      INSERT INTO programs (
        ...
        apply_url, study_mode, teaching_language, languages, intake_months,
      ...
      IF v_program_id IS NOT NULL THEN
        ...
        UPDATE program_draft SET review_status = 'published', published_at = now(),
            publish_trace_id = 'BULK-MIGRATION-ALL', published_program_id = v_program_id
        WHERE id = rec.id;
      END IF;
  ...
  VALUES ('crawl_review', 'bulk_publish_all_remaining', jsonb_build_object(
    'inserted', v_inserted, 'failed', v_failed, 'errors', v_errors
  ));

  RAISE NOTICE 'Bulk publish complete: inserted=%, failed=%', v_inserted, v_failed;
END;
$$;
