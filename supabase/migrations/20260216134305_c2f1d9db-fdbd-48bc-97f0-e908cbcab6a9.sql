...
-- ============================================================
-- DOOR 1: UniRanks Signals Expansion (Additive Only)
-- ============================================================

-- 2.1 Structured + Control columns
ALTER TABLE public.universities
  ADD COLUMN IF NOT EXISTS uniranks_profile_url text,
  ADD COLUMN IF NOT EXISTS uniranks_verified boolean,
  ADD COLUMN IF NOT EXISTS uniranks_recognized boolean,
  ADD COLUMN IF NOT EXISTS uniranks_badges text[],
  ADD COLUMN IF NOT EXISTS uniranks_top_buckets text[],
  ADD COLUMN IF NOT EXISTS uniranks_country_rank integer,
  ADD COLUMN IF NOT EXISTS uniranks_region_rank integer,
  ADD COLUMN IF NOT EXISTS uniranks_world_rank integer,
  ADD COLUMN IF NOT EXISTS uniranks_data_quality text DEFAULT 'raw',
  ADD COLUMN IF NOT EXISTS uniranks_last_reviewed_at timestamptz,
  ADD COLUMN IF NOT EXISTS uniranks_last_reviewed_by uuid;

-- 2.2 No-Loss Snapshot fields
ALTER TABLE public.universities
  ADD COLUMN IF NOT EXISTS uniranks_snapshot jsonb,
  ADD COLUMN IF NOT EXISTS uniranks_sections_present text[],
  ADD COLUMN IF NOT EXISTS uniranks_snapshot_hash text,
  ADD COLUMN IF NOT EXISTS uniranks_snapshot_at timestamptz,
  ADD COLUMN IF NOT EXISTS uniranks_snapshot_trace_id text;

-- 2.3 Indexes (Performance)
CREATE INDEX IF NOT EXISTS idx_universities_uniranks_verified ON public.universities (uniranks_verified);
CREATE INDEX IF NOT EXISTS idx_universities_uniranks_recognized ON public.universities (uniranks_recognized);
CREATE INDEX IF NOT EXISTS idx_universities_uniranks_badges ON public.universities USING GIN (uniranks_badges);
CREATE INDEX IF NOT EXISTS idx_universities_uniranks_top_buckets ON public.universities USING GIN (uniranks_top_buckets);
CREATE INDEX IF NOT EXISTS idx_universities_uniranks_country_rank ON public.universities (uniranks_country_rank);
CREATE INDEX IF NOT EXISTS idx_universities_uniranks_region_rank ON public.universities (uniranks_region_rank);

-- ============================================================
-- 3.1 RPC: upsert UniRanks signals (write)
-- ============================================================
...
BEGIN
  ...
  -- Telemetry: studio_uniranks_saved
...
-- ============================================================
-- 3.2 RPC: read UniRanks signals (for Studio)
-- ============================================================
CREATE OR REPLACE FUNCTION public.rpc_get_uniranks_signals(p_university_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  SELECT jsonb_build_object(
    'university_id', id,
    'uniranks_rank', uniranks_rank,
    'uniranks_score', uniranks_score,
    'uniranks_verified', uniranks_verified,
    'uniranks_recognized', uniranks_recognized,
    'uniranks_profile_url', uniranks_profile_url,
    'uniranks_badges', uniranks_badges,
    'uniranks_top_buckets', uniranks_top_buckets,
    'uniranks_country_rank', uniranks_country_rank,
    'uniranks_region_rank', uniranks_region_rank,
    'uniranks_world_rank', uniranks_world_rank,
    'uniranks_data_quality', uniranks_data_quality,
    'uniranks_last_reviewed_at', uniranks_last_reviewed_at,
    'uniranks_snapshot', uniranks_snapshot,
    'uniranks_sections_present', uniranks_sections_present,
    'uniranks_snapshot_hash', uniranks_snapshot_hash,
    'uniranks_snapshot_at', uniranks_snapshot_at,
    'uniranks_snapshot_trace_id', uniranks_snapshot_trace_id
  ) INTO v_result
  FROM universities
  WHERE id = p_university_id;
