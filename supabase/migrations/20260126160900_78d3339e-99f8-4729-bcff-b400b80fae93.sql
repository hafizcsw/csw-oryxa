-- Fix type mismatch: intake_months is text[] in view, not int[]. Change v_intake_months to text[].
-- Also fix entrance_exam_types filter (already text[]).
...
DECLARE
  ...
  -- FIX: intake_months is text[] in DB, not int[]
...
BEGIN
  ...
  -- Extract intake_months as text[] (DB stores as text[])
  ...
  IF v_enforce_eligibility THEN
    ...
    IF v_applicant IS NULL THEN RAISE EXCEPTION 'INVALID_INPUT: missing applicant_profile'; END IF;
    ...
    IF v_country_code IS NULL THEN
      RAISE EXCEPTION USING message='MISSING_DATA_FIELDS', detail=jsonb_build_array('admission_rules_country:requires_country_code_filter')::text;
    END IF;
    ...
    IF v_country_req IS NULL THEN
      RAISE EXCEPTION USING message='MISSING_DATA_FIELDS', detail=jsonb_build_array('admission_rules_country:missing_match:'||UPPER(v_country_code)||':curriculum='||COALESCE(v_curriculum,'')||':stream='||COALESCE(v_stream,''))::text;
    END IF;
  END IF;
