DROP VIEW IF EXISTS public.vw_university_card;

CREATE VIEW public.vw_university_card AS
SELECT u.id,
    u.name,
    u.name_ar,
    u.name_en,
    u.city,
    u.logo_url,
    COALESCE(u.main_image_url, u.hero_image_url, ( SELECT COALESCE(um.public_url, um.source_url) AS "coalesce"
           FROM university_media um
          WHERE um.university_id = u.id AND um.image_type = 'gallery'::text
          ORDER BY um.is_primary DESC NULLS LAST, um.sort_order, um.created_at
