...
-- Fix: disambiguate all column references and implement truth buckets
...
DECLARE
  ...
  _search_intel jsonb;
  _content_gaps jsonb;
  -- Cutover: events before this date are unknown_legacy
  _cutover timestamptz := '2026-03-17T16:00:00Z';
BEGIN

  -- ============ REAL TRAFFIC FILTER ============
  -- known_real = after cutover + traffic_class='real' + not admin/test
  -- OR hostname is known prod domain
  -- unknown_legacy = before cutover with no hostname
  -- known_internal = traffic_class in (internal,synthetic,dev,seed,bot) OR is_admin/is_test
  ...
  WHERE ev.created_at >= _cutover
    AND ev.traffic_class = 'real'
    AND ev.is_admin = false
    AND ev.is_test = false;

  -- ============ FUNNEL (known_real only, post-cutover) ============
  SELECT coalesce(jsonb_agg(row_to_json(f) order by f.step_order), '[]'::jsonb) INTO _funnel
  FROM (
    SELECT 'landing' as step, 1 as step_order,
      count(distinct ev.visitor_id) as visitors, 'visitor_id' as identity_domain
    FROM events ev WHERE ev.name = 'page_view' AND ev.route = '/'
      AND ev.created_at >= _cutover AND ev.traffic_class = 'real' AND ev.is_admin = false AND ev.is_test = false
    UNION ALL
    SELECT 'search', 2,
      count(distinct ev.visitor_id), 'visitor_id'
    FROM events ev WHERE (ev.name = 'search_performed' OR (ev.name = 'page_view' AND ev.route = '/search'))
      AND ev.created_at >= _cutover AND ev.traffic_class = 'real' AND ev.is_admin = false AND ev.is_test = false
    UNION ALL
    SELECT 'university_view', 3,
      count(distinct ev.visitor_id), 'visitor_id'
    FROM events ev WHERE ((ev.name = 'entity_view' AND ev.properties->>'entity_type' = 'university')
      OR (ev.name = 'page_view' AND ev.route like '/universities/%'))
      AND ev.created_at >= _cutover AND ev.traffic_class = 'real' AND ev.is_admin = false AND ev.is_test = false
    UNION ALL
    SELECT 'program_view', 4,
      count(distinct ev.visitor_id), 'visitor_id'
    FROM events ev WHERE ((ev.name = 'entity_view' AND ev.properties->>'entity_type' = 'program')
      OR (ev.name = 'page_view' AND ev.route like '/programs/%'))
      AND ev.created_at >= _cutover AND ev.traffic_class = 'real' AND ev.is_admin = false AND ev.is_test = false
    UNION ALL
    SELECT 'shortlist', 5,
      count(distinct ev.visitor_id), 'visitor_id'
    FROM events ev WHERE ev.name in ('shortlist_add','shortlist_added')
      AND ev.created_at >= _cutover AND ev.traffic_class = 'real' AND ev.is_admin = false AND ev.is_test = false
    UNION ALL
    SELECT 'register_start', 6,
      count(distinct ev.visitor_id), 'visitor_id'
    FROM events ev WHERE ev.name = 'register_start'
      AND ev.created_at >= _cutover AND ev.traffic_class = 'real' AND ev.is_admin = false AND ev.is_test = false
    UNION ALL
    SELECT 'register_complete', 7,
      (select count(*) from profiles where created_at >= _cutover), 'user_id'
    UNION ALL
    SELECT 'apply_click', 8,
      (select count(distinct user_id) from applications where created_at >= _cutover), 'user_id'
    UNION ALL
    SELECT 'doc_upload', 9,
      (select count(distinct a.user_id) from application_documents ad join applications a on a.id = ad.application_id where ad.created_at >= _cutover), 'user_id'
    UNION ALL
    SELECT 'payment', 10, 0, 'user_id'
  ) f;

  -- ============ ENGAGEMENT (known_real, post-cutover) ============
  SELECT jsonb_build_object(
    'top_pages_by_views', coalesce((
      ...
      from (
        ...
        from events ev where ev.name = 'page_view' and ev.route is not null
          and ev.created_at >= _cutover
          and ev.traffic_class = 'real' and ev.is_admin = false and ev.is_test = false
    ...
    'hourly_pattern', coalesce((
      ...
      from (
        ...
        from events ev where ev.name = 'page_view'
          and ev.created_at >= _cutover
          and ev.traffic_class = 'real' and ev.is_admin = false and ev.is_test = false
        group by 1
      ) t
  ...
  -- ============ UNIVERSITY INTEL (known_real, post-cutover) ============
  SELECT jsonb_build_object(
    'top_by_views', coalesce((
      select jsonb_agg(row_to_json(t) order by t.views desc)
      from (
        with ev_views as (
          select ev.properties->>'entity_slug' as slug, ev.visitor_id
          from events ev
        ...
        rt_views as (
          select replace(ev.route, '/universities/', '') as slug, ev.visitor_id
          from events ev
          where ev.name = 'page_view' and ev.route like '/universities/%' and ev.route not like '/universities/%/%'
            and ev.created_at >= _cutover
            and ev.traffic_class = 'real' and ev.is_admin = false and ev.is_test = false
            and not exists (
              select 1 from ev_views evv
              where evv.slug = replace(ev.route, '/universities/', '')
                and evv.visitor_id = ev.visitor_id
            )
        ),
        combined as (
          select slug, visitor_id from ev_views
          union all
          select slug, visitor_id from rt_views
        )
        select
    ...
    'top_by_shortlist', coalesce((
      ...
      from (
        select coalesce(u.name_ar,'') as name_ar, coalesce(u.name_en,'') as name_en,
          coalesce(u.slug,'') as slug, ev.properties->>'entity_id' as entity_id,
          count(*) as adds, count(distinct ev.visitor_id) as unique_users
        from events ev
        left join universities u on u.id::text = ev.properties->>'entity_id'
        where ev.name in ('shortlist_add','shortlist_added')
          and ev.created_at >= _cutover
          and ev.traffic_class = 'real' and ev.is_admin = false and ev.is_test = false
        group by ev.properties->>'entity_id', u.name_ar, u.name_en, u.slug
        limit 10
      ) t
    ), '[]'::jsonb),
    'top_programs_by_views', coalesce((
      ...
      from (
        ...
        rt_views as (
          select replace(ev.route, '/programs/', '') as slug, ev.visitor_id
          from events ev
          where ev.name = 'page_view' and ev.route like '/programs/%'
            ...
            and not exists (
              select 1 from ev_views evv
              where evv.slug = replace(ev.route, '/programs/', '')
                and evv.visitor_id = ev.visitor_id
            )
        ),
        combined as (
  ...
  SELECT jsonb_build_object(
    ...
    'search_to_click_pct', coalesce((
      select round(
        count(distinct ev.visitor_id) filter(where ev.name = 'search_result_click')::numeric /
    ...
    'total_searches_30d', (
      select count(*) from events ev where ev.name = 'search_performed'
        and ev.created_at >= _cutover
        and ev.traffic_class = 'real' and ev.is_admin = false and ev.is_test = false
    ),
  ...
  SELECT jsonb_build_object(
    'universities_missing_tuition', coalesce((
      ...
      from (
        ...
        from events ev
        join universities u on u.slug = replace(ev.route, '/universities/', '')
