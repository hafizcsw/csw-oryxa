...
DROP FUNCTION IF EXISTS rpc_publish_program_batch_search(UUID);

CREATE FUNCTION rpc_publish_program_batch_search(p_batch_id UUID)
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- FIX: LOWER() on languages to satisfy program_languages_lang_format constraint
      v_languages := COALESCE(
        ARRAY(SELECT LOWER(jsonb_array_elements_text(rec.extracted_json->'languages'))),
        ARRAY['en']
      );
