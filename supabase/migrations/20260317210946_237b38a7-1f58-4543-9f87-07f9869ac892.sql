...
DECLARE
  ...
  -- Reusable real-lane filter (excludes admin routes AND admin/test/staff flags)
  -- _rl = "real lane" WHERE clause fragment
BEGIN
  -- ============ OVERVIEW ============
  ...
  FROM events ev
  WHERE ev.created_at >= _cutover AND COALESCE(ev.traffic_class,'real') IN ('real','known_real') AND COALESCE(ev.is_admin,false)=false AND COALESCE(ev.is_test,false)=false AND COALESCE(ev.is_staff,false)=false AND COALESCE(ev.route,'') NOT LIKE '/admin%';

  -- ============ SINGLE FUNNEL ============
  -- Landing = any page_view (excluding admin)
  -- Search = page_view on /universities route (exact, with locale prefix support)
  -- search_performed = explicit search_performed event
  ...
  -- ============ 3 FUNNELS ============
  -- Discovery funnel step definitions:
  --   landing: any page_view (not admin) = "visitor entered the site"
  --   search_page: page_view with route matching /universities (exact with locale)
  --   search_performed: explicit search_performed event
  --   university_view: entity_view with entity_type=university
  --   program_view: entity_view with entity_type=program
  --   shortlist: shortlist_added event
  SELECT jsonb_build_array(
    jsonb_build_object('name','discovery','steps', jsonb_build_array(
      jsonb_build_object('step','landing','step_order',1,'identity_domain','visitor_id','step_definition','page_view excluding /admin','visitors',(SELECT count(distinct visitor_id) FROM events WHERE created_at >= _cutover AND COALESCE(traffic_class,'real') IN ('real','known_real') AND COALESCE(is_admin,false)=false AND COALESCE(is_test,false)=false AND COALESCE(is_staff,false)=false AND COALESCE(route,'') NOT LIKE '/admin%' AND created_at >= now()-interval '30 days' AND name = 'page_view')),
      jsonb_build_object('step','search_page','step_order',2,'identity_domain','visitor_id','step_definition','page_view on /universities route','visitors',(SELECT count(distinct visitor_id) FROM events WHERE created_at >= _cutover AND COALESCE(traffic_class,'real') IN ('real','known_real') AND COALESCE(is_admin,false)=false AND COALESCE(is_test,false)=false AND COALESCE(is_staff,false)=false AND created_at >= now()-interval '30 days' AND name = 'page_view' AND COALESCE(route, properties->>'route') ~ '(/[a-z]{2})?/universities(\?|$)')),
      jsonb_build_object('step','search_performed','step_order',3,'identity_domain','visitor_id','step_definition','search_performed event','visitors',(SELECT count(distinct visitor_id) FROM events WHERE created_at >= _cutover AND COALESCE(traffic_class,'real') IN ('real','known_real') AND COALESCE(is_admin,false)=false AND COALESCE(is_test,false)=false AND COALESCE(is_staff,false)=false AND created_at >= now()-interval '30 days' AND name = 'search_performed')),
      jsonb_build_object('step','university_view','step_order',4,'identity_domain','visitor_id','step_definition','entity_view entity_type=university','visitors',(SELECT count(distinct visitor_id) FROM events WHERE created_at >= _cutover AND COALESCE(traffic_class,'real') IN ('real','known_real') AND COALESCE(is_admin,false)=false AND COALESCE(is_test,false)=false AND COALESCE(is_staff,false)=false AND created_at >= now()-interval '30 days' AND name = 'entity_view' AND properties->>'entity_type' = 'university')),
      jsonb_build_object('step','program_view','step_order',5,'identity_domain','visitor_id','step_definition','entity_view entity_type=program','visitors',(SELECT count(distinct visitor_id) FROM events WHERE created_at >= _cutover AND COALESCE(traffic_class,'real') IN ('real','known_real') AND COALESCE(is_admin,false)=false AND COALESCE(is_test,false)=false AND COALESCE(is_staff,false)=false AND created_at >= now()-interval '30 days' AND name = 'entity_view' AND properties->>'entity_type' = 'program')),
      jsonb_build_object('step','shortlist','step_order',6,'identity_domain','visitor_id','step_definition','shortlist_added event','visitors',(SELECT count(distinct visitor_id) FROM events WHERE created_at >= _cutover AND COALESCE(traffic_class,'real') IN ('real','known_real') AND COALESCE(is_admin,false)=false AND COALESCE(is_test,false)=false AND COALESCE(is_staff,false)=false AND created_at >= now()-interval '30 days' AND name = 'shortlist_added'))
    )),
  ...
  -- ============ ENGAGEMENT ============
  SELECT jsonb_build_object(
    'hourly_pattern', COALESCE((SELECT jsonb_agg(row_to_json(h) ORDER BY h.hr) FROM (SELECT extract(hour from e4.created_at)::int as hr, count(distinct e4.visitor_id) as visitors FROM events e4 WHERE e4.created_at >= _cutover AND COALESCE(e4.traffic_class,'real') IN ('real','known_real') AND COALESCE(e4.is_admin,false)=false AND COALESCE(e4.is_test,false)=false AND COALESCE(e4.is_staff,false)=false AND COALESCE(e4.route,'') NOT LIKE '/admin%' AND e4.created_at >= now()-interval '24 hours' GROUP BY 1) h), '[]'::jsonb),
    'top_pages_by_views', COALESCE((SELECT jsonb_agg(row_to_json(pg) ORDER BY pg.views DESC) FROM (SELECT COALESCE(e5.route, e5.properties->>'route') as page_route, count(*) as views, count(distinct e5.visitor_id) as unique_visitors FROM events e5 WHERE e5.created_at >= _cutover AND e5.name = 'page_view' AND COALESCE(e5.traffic_class,'real') IN ('real','known_real') AND COALESCE(e5.is_admin,false)=false AND COALESCE(e5.is_test,false)=false AND COALESCE(e5.is_staff,false)=false AND COALESCE(e5.route,'') NOT LIKE '/admin%' AND e5.created_at >= now()-interval '7 days' AND COALESCE(e5.route, e5.properties->>'route') IS NOT NULL GROUP BY COALESCE(e5.route, e5.properties->>'route') ORDER BY views DESC LIMIT 10) pg), '[]'::jsonb)
  ) INTO _engagement;
  ...
  SELECT jsonb_build_object(
    ...
    'top_by_views', COALESCE((SELECT jsonb_agg(row_to_json(u) ORDER BY u.views DESC) FROM (
      SELECT COALESCE(uni.name_ar, uni.name_en, sub.entity_slug, sub.entity_id) as name_ar, COALESCE(uni.name_en, uni.name_ar, sub.entity_slug) as name_en, sub.entity_slug as slug, count(*) as views, count(distinct sub.vid) as unique_visitors
      ...
      LEFT JOIN universities uni ON uni.id::text = sub.entity_id
      GROUP BY COALESCE(uni.name_ar, uni.name_en, sub.entity_slug, sub.entity_id), COALESCE(uni.name_en, uni.name_ar, sub.entity_slug), sub.entity_slug ORDER BY views DESC LIMIT 10
    ) u), '[]'::jsonb),
    'top_by_shortlist', COALESCE((SELECT jsonb_agg(row_to_json(u) ORDER BY u.adds DESC) FROM (
      SELECT COALESCE(uni.name_ar, uni.name_en, sub.entity_slug, sub.entity_id) as name_ar, COALESCE(uni.name_en, uni.name_ar, sub.entity_slug) as name_en, sub.entity_id as entity_id, count(*) as adds, count(distinct sub.vid) as unique_users
      ...
      LEFT JOIN universities uni ON uni.id::text = sub.entity_id
      GROUP BY COALESCE(uni.name_ar, uni.name_en, sub.entity_slug, sub.entity_id), COALESCE(uni.name_en, uni.name_ar, sub.entity_slug), sub.entity_id ORDER BY adds DESC LIMIT 10
    ) u), '[]'::jsonb),
    'top_programs_by_views', COALESCE((SELECT jsonb_agg(row_to_json(p2) ORDER BY p2.views DESC) FROM (
      SELECT COALESCE(prog.title_ar, prog.title, sub.entity_slug, sub.entity_id) as program_title, COALESCE(uni.name_ar, uni.name_en, '') as university_name, sub.entity_slug as prog_slug, count(*) as views, count(distinct sub.vid) as unique_visitors
      ...
      LEFT JOIN programs prog ON prog.id::text = sub.entity_id
      LEFT JOIN universities uni ON uni.id = prog.university_id
      GROUP BY COALESCE(prog.title_ar, prog.title, sub.entity_slug, sub.entity_id), COALESCE(uni.name_ar, uni.name_en, ''), sub.entity_slug ORDER BY views DESC LIMIT 10
    ) p2), '[]'::jsonb)
  ) INTO _uni_intel;
  ...
  SELECT jsonb_build_object(
    ...
    'top_country_filters', COALESCE((SELECT jsonb_agg(row_to_json(f) ORDER BY f.uses DESC) FROM (SELECT COALESCE(e9.properties->'filters'->>'country_slug', e9.properties->'filters'->>'country') as filter_val, count(*) as uses, count(distinct e9.visitor_id) as unique_users FROM events e9 WHERE e9.name='search_performed' AND e9.created_at >= _cutover AND COALESCE(e9.traffic_class,'real') IN ('real','known_real') AND COALESCE(e9.is_admin,false)=false AND COALESCE(e9.is_test,false)=false AND e9.created_at >= now()-interval '30 days' AND COALESCE(e9.properties->'filters'->>'country_slug', e9.properties->'filters'->>'country') IS NOT NULL GROUP BY filter_val ORDER BY uses DESC LIMIT 10) f), '[]'::jsonb),
    'top_degree_filters', COALESCE((SELECT jsonb_agg(row_to_json(f) ORDER BY f.uses DESC) FROM (SELECT COALESCE(e10.properties->'filters'->>'degree_id', e10.properties->'filters'->>'degree') as filter_val, count(*) as uses FROM events e10 WHERE e10.name='search_performed' AND e10.created_at >= _cutover AND COALESCE(e10.traffic_class,'real') IN ('real','known_real') AND COALESCE(e10.is_admin,false)=false AND COALESCE(e10.is_test,false)=false AND e10.created_at >= now()-interval '30 days' AND COALESCE(e10.properties->'filters'->>'degree_id', e10.properties->'filters'->>'degree') IS NOT NULL GROUP BY filter_val ORDER BY uses DESC LIMIT 10) f), '[]'::jsonb)
  ) INTO _search_intel;

  -- ============ CONTENT GAPS ============
  SELECT jsonb_build_object(
    'universities_missing_tuition', COALESCE((SELECT jsonb_agg(row_to_json(u)) FROM (SELECT COALESCE(uni.name_ar, uni.name_en, uni.slug, uni.id::text) as name_ar, uni.slug, 0 as views FROM universities uni WHERE NOT EXISTS (SELECT 1 FROM programs prog WHERE prog.university_id = uni.id AND prog.tuition_usd_min IS NOT NULL) ORDER BY uni.name_ar NULLS LAST LIMIT 10) u), '[]'::jsonb),
    'programs_missing_deadlines', COALESCE((SELECT jsonb_agg(row_to_json(p2)) FROM (SELECT COALESCE(prog.title_ar, prog.title, prog.id::text) as title, COALESCE(uni.name_ar, uni.name_en, uni.slug, uni.id::text) as uni_name, uni.slug as uni_slug, 0 as views FROM programs prog JOIN universities uni ON uni.id = prog.university_id WHERE prog.deadlines IS NULL ORDER BY uni.name_ar NULLS LAST LIMIT 10) p2), '[]'::jsonb),
