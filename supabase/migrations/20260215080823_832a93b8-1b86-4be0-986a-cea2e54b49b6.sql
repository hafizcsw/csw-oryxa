...
-- ============================================================
-- FIX: Search Publish RPC - two-step insert to satisfy PUBLISH_GATE_V3
-- Step 1: INSERT as draft (bypasses gate trigger)
-- Step 2: INSERT program_languages
-- Step 3: UPDATE to published (gate validates and passes)
-- ============================================================
...
BEGIN
  ...
  LOOP
    BEGIN
      -- ====== GATE CHECK: mandatory fields ======
      IF rec.title IS NULL OR rec.degree_level IS NULL OR rec.duration_months IS NULL THEN
        v_skipped := v_skipped + 1;
        CONTINUE;
      END IF;

      -- Map degree_level → degree_id
      SELECT d.id INTO v_degree_id FROM degrees d
      WHERE d.slug = LOWER(COALESCE(rec.degree_level, '')) LIMIT 1;
      IF v_degree_id IS NULL THEN
        SELECT d.id INTO v_degree_id FROM degrees d
        WHERE LOWER(COALESCE(rec.degree_level, '')) LIKE '%' || d.slug || '%' LIMIT 1;
      END IF;
      ...
      WHERE d.slug = LOWER(COALESCE(rec.extracted_json->>'discipline_hint', '')) LIMIT 1;
      ...
      -- University readiness check (city, country, monthly_living, guidance)
      SELECT EXISTS(
      ...
      -- ====== STEP 1: Upsert as DRAFT (bypasses PUBLISH_GATE_V3) ======
      ...
      -- ====== STEP 3: Promote to PUBLISHED (triggers PUBLISH_GATE_V3) ======
      UPDATE programs SET
