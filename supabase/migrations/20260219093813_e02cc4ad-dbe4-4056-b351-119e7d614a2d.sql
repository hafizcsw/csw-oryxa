...
-- Fix: program_drafts → program_draft (correct table name)
...
BEGIN
  ...
  WITH filtered AS (
    SELECT
      ...
      COALESCE((SELECT count(*) FROM program_draft pd WHERE pd.university_id = u.id), 0)::int AS program_draft_count,
