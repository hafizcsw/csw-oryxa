...
BEGIN
  ...
  FROM (
    ...
    FROM events WHERE name IN ('search_performed','page_view') AND created_at > c30d
    AND (name='search_performed' OR (name='page_view' AND COALESCE(route,'') LIKE '%search%'))
    UNION ALL
  ...
  SELECT jsonb_build_object(
    ...
    'top_programs_by_views', CASE WHEN has_entity_views THEN
      COALESCE((
        SELECT jsonb_agg(row_to_json(r)::jsonb)
        FROM (
          SELECT COALESCE(p.title, pv.pid) as program_title, u.name as university_name,
                 COALESCE(p.program_slug, p.id::text, pv.pid) as prog_slug,
                 pv.views, pv.unique_visitors
          FROM (
            SELECT COALESCE(e.properties->>'entity_id', e.properties->>'entity_slug') as pid,
            ...
            GROUP BY pid ORDER BY views DESC LIMIT 20
          ) pv
          LEFT JOIN programs p ON p.id::text = pv.pid OR p.program_slug = pv.pid
    ...
    ELSE
      COALESCE((
        SELECT jsonb_agg(row_to_json(r)::jsonb)
        FROM (
          SELECT COALESCE(p.title, pv.pslug) as program_title, u.name as university_name,
                 COALESCE(p.program_slug, pv.pslug) as prog_slug,
                 pv.views, pv.unique_visitors
          FROM (
            SELECT REGEXP_REPLACE(COALESCE(route,''), '^/programs/([^/]+).*$', '\1') as pslug,
                   COUNT(*) as views, COUNT(DISTINCT visitor_id) as unique_visitors
            FROM events WHERE name='page_view' AND created_at > c30d AND COALESCE(route,'') ~ '^/programs/[^/]+/?$'
            GROUP BY pslug ORDER BY views DESC LIMIT 20
          ) pv
          LEFT JOIN programs p ON p.program_slug = pv.pslug
          LEFT JOIN universities u ON u.id = p.university_id
          ORDER BY pv.views DESC
        ) r
      ), '[]')
  ...
  SELECT jsonb_build_object(
    ...
    'attribution_method', 'visitor_based'
  ) INTO search_intel;
